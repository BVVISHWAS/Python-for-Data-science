...
classifier = SVC(C= 0.2, kernel= 'rbf')
...
predicted_SVC = classifier.predict(X1)

predicted_SVC = pd.DataFrame(predicted_SVC)

Final_output = pd.concat([PASSinger, predicted_SVC], axis=1)

Final_output.to_csv("D:\\ML\\TITANIC\\New\\Submission_Titanic_SVM.csv", index=False)
